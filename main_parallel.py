...
def try_range_hypothesis(start, end):
  ...
  print(f"Finished testing from {start} to {end}")
  return True

def try_range_hypothesis_multithread(start, end):
  size = end - start
  chunk_size = 100000
  start_time = time.time()

  # p = Pool(processes=size // chunk_size)

  # p.map(try_range_hypothesis, [(i, i + chunk_size) for i in range(start, end, chunk_size)])
  ...
  for i in range(start, end, chunk_size):
    thread = multiprocessing.Process(target=try_range_hypothesis, args=(i, i + chunk_size))
...
if __name__ == "__main__":
  try_range_hypothesis_multithread(1, 1_000_000)